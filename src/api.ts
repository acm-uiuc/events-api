...
APIRouter.post('/private/events/create', (req: Request, res: Response) => {
    ...
    event.save((err, doc) => {
        if (err || !doc) {
            return res.status(500).json({error: "Could not save the document.", success: false})
        }
        return res.json({error: "Event created.", id: doc._id.toString(), success: true})
    })
})
...
APIRouter.get('/public/events/all', (req: Request, res: Response) => {
    ...
    Event.find({}, '-__v', (err: mongoose.Error, doc: mongoose.Document) => {
        if (err || !doc) {
            return res.status(500).json({error: "Could not find the events.", success: false})
        }
        return res.send(doc)
    })
})
...
APIRouter.get('/public/events/:id', (req: Request, res: Response) => {
    const Event = mongoose.model('Event', EventSchema, "events");
    Event.findById(req.params.id, '-__v', (err: mongoose.Error, doc: mongoose.Document) => {
        if (err || !doc) {
            return res.status(500).json({error: "Could not find the document.", success: false})
        }
        return res.send(doc)
    })
})
...
APIRouter.delete('/private/events/:id', (req: Request, res: Response) => {
    const Event = mongoose.model('Event', EventSchema, "events");
    Event.findByIdAndDelete(req.params.id, (err: mongoose.Error, doc: mongoose.Document) => {
        if (err || !doc) {
            return res.status(500).json({error: "Could not find the document.", success: false})
        }
...
APIRouter.patch('/private/events/:id', (req: Request, res: Response) => {
    const Event = mongoose.model('Event', EventSchema, "events");
    Event.findByIdAndUpdate(req.params.id, req.body, (err: mongoose.Error, doc: mongoose.Document) => {
        if (err || !doc) {
            return res.status(500).json({error: "Could not find the document.", success: false})
        }
